  ...
  return (
    <Layout title="My Account">
      <div className="container mx-auto px-4 py-6 sm:py-8">
        <h1 className="mb-5 font-serif text-2xl font-medium text-[#2E1F14] sm:mb-6 sm:text-3xl">My Account</h1>

        <div className="flex flex-col gap-5 md:flex-row md:gap-6">
          <nav className="-mx-4 flex w-[calc(100%+2rem)] gap-2 overflow-x-auto px-4 pb-1 md:mx-0 md:w-32 md:flex-col md:overflow-visible md:px-0 md:pb-0">
            {tabs.map((t) => {
              const Icon = t.icon;
              ...
              return (
                <button
                  ...
                  onClick={() => setTab(t.id)}
                  className={cn(
                    "flex min-h-11 min-w-[92px] shrink-0 items-center justify-center gap-1.5 rounded-xl px-3 py-2 text-xs font-medium transition-colors md:min-w-0 md:justify-start md:gap-2 md:px-3 md:text-sm",
                    tab === t.id
            ...
            {tab === "orders" && (
              <div className="space-y-3">
                ...
                {ordersLoading ? (
                  ...
                  <>
                    {orders.map((order) => (
                      <div key={order.id} className="min-w-0 rounded-2xl border border-[#E8DDD0] bg-white p-3.5 shadow-[0_4px_18px_rgba(46,31,20,0.04)] sm:p-4">
                        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                          <button
                            onClick={() => navigate(`/dashboard/user/orders/${order.id}`)}
                            className="min-w-0 truncate rounded-md text-left font-mono text-xs underline underline-offset-2 sm:text-sm"
                  ...
                  <>
                    {addressesLoading ? (
                      ...
                      <div className="grid gap-4 md:grid-cols-2">
                        {addresses.map((addr) => (
                          <div key={addr.id} className="min-w-0 rounded-2xl border border-[#E8DDD0] bg-white p-3.5 shadow-[0_4px_18px_rgba(46,31,20,0.04)] sm:p-4">
